...
$spplace = [];
if ($model->district) {
    $spplace = Street::find()->where(['sp_district' => $model->birth_place_id])->asArray()->all();
}
...
<div class="kus-form">
    ...
    <?= $form->field($model, 'birth_country_id')->dropDownList(ArrayHelper::map($country, 'sp_id','sp_name_'.Yii::$app->language)) ?>
    ...
    <?= $form->field($model, 'date_endocument')->widget(DatePicker::classname(), [
      'language' => 'ru',
      'pluginOptions' => [
          'format' => 'yyyy-mm-dd',
          'todayHighlight' => true
   ...
   <?=  $form->field($model, 'senstatus')->textInput() ?>
    ...
    <?= $form->field($model, 'endate')->widget(DatePicker::classname(), [
      'language' => 'ru',
      'pluginOptions' => [
          'format' => 'yyyy-mm-dd',
          'todayHighlight' => true
